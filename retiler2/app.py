...
def orders():
    try:
        # Read orders directly from Excel
        orders_df = pd.read_excel('orders.xlsx', sheet_name='Sheet1')
        ...
        if status_filter:
            filtered_orders = [o for o in filtered_orders 
                             if str(o.get('Status', '')).lower() == status_filter.lower()]
        
        if date_from:
            try:
                date_from = datetime.strptime(date_from, '%Y-%m-%d')
                filtered_orders = [o for o in filtered_orders 
                                 if datetime.strptime(str(o.get('OrderDate', '')), '%Y-%m-%d') >= date_from]
            except:
                pass  # Skip date filtering if format is wrong
        
        if date_to:
            try:
                date_to = datetime.strptime(date_to, '%Y-%m-%d')
                filtered_orders = [o for o in filtered_orders 
                                 if datetime.strptime(str(o.get('OrderDate', '')), '%Y-%m-%d') <= date_to]
            except:
                pass  # Skip date filtering if format is wrong
