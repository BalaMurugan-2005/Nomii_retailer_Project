...
def get_user_orders(retailer_id):
    try:
        ...
        return df[df['RetailerID'] == retailer_id].to_dict('records')
    except Exception as e:
        print(f"Error getting user orders: {e}")
        return []

def get_product_suggestions(retailer_id):
    try:
...
def generate_restock_predictions(retailer_id):
    try:
        orders_df = pd.read_excel(ORDERS_FILE)
        retailer_orders = orders_df[orders_df['RetailerID'] == retailer_id]
        
        if retailer_orders.empty:
            return []
            
        freq_products = retailer_orders['ProductName'].value_counts().head(5).index.tolist()
        recent_orders = retailer_orders.sort_values('OrderDate', ascending=False)
...
def generate_combo_suggestions(retailer_id):
    try:
        orders_df = pd.read_excel(ORDERS_FILE)
        retailer_orders = orders_df[orders_df['RetailerID'] == retailer_id]
        ...
        for order_id in order_ids:
            products = retailer_orders[retailer_orders['OrderID'] == order_id]['ProductName'].tolist()
...
def generate_weekly_insights(retailer_id):
    try:
        orders_df = pd.read_excel(ORDERS_FILE)
        retailer_orders = orders_df[orders_df['RetailerID'] == retailer_id]
        ...
        weekly_spending = retailer_orders.groupby(
            retailer_orders['OrderDate'].dt.to_period('W'))['Total'].sum().reset_index()
        ...
        avg_order_value = float(retailer_orders['Total'].mean())  # Convert to native float
        order_count = int(len(retailer_orders['OrderID'].unique()))  # Convert to native int
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user is not None and check_password_hash(user['Password'], password):
                session['retailer_id'] = int(user['RetailerID'])  # Ensure native int
...
def signup():
    if request.method == 'POST':
        try:
            ...
            new_user = {
                'RetailerID': get_next_id(USERS_FILE, 'RetailerID'),
...
def dashboard():
    ...
    orders = get_user_orders(retailer_id)
    suggestions = get_product_suggestions(retailer_id)
    restock_predictions = generate_restock_predictions(retailer_id)
    combo_suggestions = generate_combo_suggestions(retailer_id)
    weekly_insights = generate_weekly_insights(retailer_id)
...
def products():
    ...
    try:
        ...
        df['ProductID'] = df['ProductID'].astype(int)  # Ensure native int
        df['Price'] = df['Price'].astype(float)  # Ensure native float
...
def add_to_cart():
    ...
    try:
        df = pd.read_excel(PRODUCTS_FILE)
        product = df[df['ProductID'] == int(product_id)].iloc[0]
        ...
        if not found:
            cart.append({
                'ProductID': int(product_id),  # Ensure native int
...
def place_order():
    ...
    try:
        ...
        update_rewards(retailer_id, total_amount)
...
def update_rewards(retailer_id, amount):
    try:
        df = pd.read_excel(REWARDS_FILE)
        retailer_rewards = df[df['RetailerID'] == retailer_id]
        
        points_earned = int(float(amount) / 10)  # Ensure proper calculation
        
        if retailer_rewards.empty:
            new_rewards = {
                'RetailerID': retailer_id,
            ...
            idx = retailer_rewards.index[0]
            df.at[idx, 'Points'] = int(df.at[idx, 'Points']) + points_earned  # Ensure native int
            
            if int(df.at[idx, 'Points']) >= 100 and int(df.at[idx, 'Level']) == 1:
                df.at[idx, 'Level'] = 2
                df.at[idx, 'Badges'] = 'Bronze'
            elif int(df.at[idx, 'Points']) >= 500 and int(df.at[idx, 'Level']) == 2:
                df.at[idx, 'Level'] = 3
                df.at[idx, 'Badges'] = 'Silver'
            elif int(df.at[idx, 'Points']) >= 1000 and int(df.at[idx, 'Level']) == 3:
                df.at[idx, 'Level'] = 4
                df.at[idx, 'Badges'] = 'Gold'
...
def orders():
    ...
    if date_from:
        ...
        orders = [o for o in orders if datetime.strptime(o['OrderDate'], '%Y-%m-%d %H:%M:%S') >= date_from]
    
    if date_to:
        date_to = datetime.strptime(date_to, '%Y-%m-%d')
        orders = [o for o in orders if datetime.strptime(o['OrderDate'], '%Y-%m-%d %H:%M:%S') <= date_to]
    
    try:
        delivery_df = pd.read_excel(DELIVERY_STATUS_FILE)
        delivery_statuses = {int(d['OrderID']): d for d in delivery_df.to_dict('records')}  # Ensure OrderID is int
    except Exception as e:
        print(f"Error getting delivery statuses: {e}")
        delivery_statuses = {}
...
def ai_assistant():
    ...
    if request.method == 'POST':
        ...
        if 'track' in query and 'order' in query:
            last_order = sorted(get_user_orders(session['retailer_id']), key=lambda x: x['OrderDate'], reverse=True)[0]
            ...
            suggestions = get_product_suggestions(session['retailer_id'])
            ...
            predictions = generate_restock_predictions(session['retailer_id'])
            if predictions:
            ...
            combos = generate_combo_suggestions(session['retailer_id'])
            if combos:
...
def profile():
    ...
    try:
        df = pd.read_excel(USERS_FILE)
        user = df[df['RetailerID'] == retailer_id].iloc[0].to_dict()
        
        rewards_df = pd.read_excel(REWARDS_FILE)
        rewards = rewards_df[rewards_df['RetailerID'] == retailer_id].iloc[0].to_dict() if not rewards_df[rewards_df['RetailerID'] == retailer_id].empty else None
        
        money_df = pd.read_excel(MONEY_SPENT_FILE)
        total_spent = float(money_df[money_df['RetailerID'] == retailer_id]['Amount'].sum()) if not money_df[money_df['RetailerID'] == retailer_id].empty else 0.0
